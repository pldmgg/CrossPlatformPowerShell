function Get-LDAPUserAndGroups {
    [CmdletBinding()]
    Param(
    ...
    if ($PSVersionTable.Platform -eq "Unix") {
        ...
        foreach ($Line in $UserAndGroupsPrep) {
            if ($Line -match 'dn:') {
                if ($Collection.Count -gt 0) {
                    $null = $UserAndGroupsPrepSorted.Add($Collection)
                }
        ...
        $UserAndGroups = foreach ($Collection in $UserAndGroupsPrepSorted) {
            try {
                $User = $($($Collection -match 'dn:') -split ',')[0] -replace 'dn:[\s]+CN='
                $GroupsPrep = $Collection -match 'memberof:'
                $Groups = foreach ($GroupLine in $GroupsPrep) {
                    $($GroupLine -split ',')[0] -replace 'memberof:[\s]+CN='
                }
            }
            catch {
                continue
        ...
        try {
            ...
            $LDAPSearcher.Filter = "(&(objectClass=user)(!(objectClass=computer)))"
            $LDAPSearcher.SizeLimit = 0
            $LDAPSearcher.PageSize = 250
            $UserAndGroupsPrep = $LDAPSearcher.FindAll() | foreach {$_.GetDirectoryEntry()}

            if ($ObjectCount -gt 0) {
                $UserAndGroupsPrep = $UserAndGroupsPrep[0..$($ObjectCount-1)]
            }

            $UserAndGroups = foreach ($DEObject in $UserAndGroupsPrep) {
                $User = $($DEObject.Name | Out-String).Trim()
                $Groups = $DEObject.memberof | foreach {$($_ -split ',')[0] -replace 'CN='}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUgVjj87c9KkBUtrPILJQ4JNJR
# sEugggn9MIIEJjCCAw6gAwIBAgITawAAAB/Nnq77QGja+wAAAAAAHzANBgkqhkiG
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFNXFUAfOib5xulCs
# f+1kEJeE25TaMA0GCSqGSIb3DQEBAQUABIIBAGC7F/cd6EISy450BvMAfXyaL4gU
# qirmxJa4s3VdOxmbwE9bKR571PFOKUWqfZsIM8icsFKmGrrQwpE67EsPUwNoy6TU
# F1DchNLfwDSehkTQNgS7ol6P4aSbLuF8xBlZJekaArC2vhzi6jDaxQt9o/v9lA+X
# +YZ/kEIqgTA1PtFXlXZJjTuEb1iylRobDS4JoTOszVq2vbmCn9jQfFrMDokCRaVk
# UrK1wIoiLkwhyxYGJapnMjwg8jJBLOBc237ZdU4tz2m5/D1qkO/DvtqewtYORlGG
# H0rMhxdpRlcc5rCLyXhZ1V8j2uSqUM6c8xfFvvx+MhVtQSc7urrtVjDCCEw=
